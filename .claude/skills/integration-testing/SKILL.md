...
### Prerequisites
1. Apple Contacts configured with at least one account
...
# 3. Require CONTACTS_TEST_GROUP environment variable
...
2. **Account not found** — Verify `CONTACTS_TEST_GROUP` matches an actual configured account name in Contacts.app
3. **Timeout** — Large mailboxes may exceed 60s default. Use `AppleContactsConnector(timeout=120)`
