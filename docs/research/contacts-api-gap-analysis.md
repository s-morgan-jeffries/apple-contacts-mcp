...
2. **`modificationDate` / `creationDate` durability** — these are not documented `CNContactKey` constants but are accessible via runtime selectors. Are they stable across macOS 26 → 27? Re-check after each macOS major release. If they break, AppleScript is the fallback. **Resolved 2026-05-18 (#33):** durability regression test landed at [`tests/integration/test_modification_dates.py`](../../tests/integration/test_modification_dates.py); both selectors verified bridged on the current macOS. Re-run via `make test-integration` after each macOS major to catch regressions early — failure includes a pointer to the AppleScript fallback to wire.
3. **vCard 4.0 export** — no native path. **Resolved 2026-05-09 (option c): emit Apple's vCard 3.0 verbatim. See [vcard-version-decision.md](./vcard-version-decision.md) for the full reasoning, the empirical probe of Apple's output (including the `X-APPLE-OMIT-YEAR` hack for year-less BDAYs), and the documented limitations.**
4. **Phone label translation** — should the MCP layer translate `_$!<Mobile>!$_` → `"mobile"` on read? **Resolved 2026-05-09 (12-entry English table):** read-side translation already exists via Apple's `localizedStringForLabel:`. The write side now accepts a single `label` field with human forms, Apple tokens, or custom strings — see [vcard-version-decision.md](./vcard-version-decision.md)'s sibling [label-translation-decision.md](./label-translation-decision.md) for the empirical probe and the input-contract redesign.
...
6. **Authorization revocation mid-process** — if the user revokes during operation, the next call returns empty results / error silently in some cases. Build a re-check at the entry of every tool, not just at start-up. Captured as a v0.4.0 hardening task. **Resolved 2026-05-13 (#37):** every data tool already re-checks at entry; v0.4.0 added a post-call `_verify_authorization_still_granted()` that fires on suspicious results (empty list, None contact) for reads and unconditionally after destructive saves. Closes the race window between the entry check and the CN call. Manual integration runbook at [`tests/integration/test_authorization_revocation.md`](../../tests/integration/test_authorization_revocation.md).
