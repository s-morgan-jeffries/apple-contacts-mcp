...
| `rate_limited` | A tool exceeded its tier's sliding-window limit (see `OPERATION_TIERS` and `TIER_LIMITS` in `security.py`). **Built in #35 but not wired into any tool yet — #46 will hook `check_rate_limit` into the destructive paths.** Once wired, callers retry after the window passes (60 s currently). The error message includes the operation, limit, and tier name. | — |
