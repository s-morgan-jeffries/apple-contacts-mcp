...
help:
	...
	@echo "  make test-integration - Run integration tests (requires Contacts.app)"
...
test-integration:
	CONTACTS_TEST_MODE=true uv run pytest tests/integration/ --run-integration -v

test-e2e:
	CONTACTS_TEST_MODE=true uv run pytest tests/e2e/ -v

benchmark:
	CONTACTS_TEST_MODE=true uv run pytest tests/benchmarks/ --run-benchmark -v -s

benchmark-baseline:
	@echo "Re-capturing baselines into tests/benchmarks/baseline.json..."
	CONTACTS_TEST_MODE=true uv run pytest tests/benchmarks/ --run-benchmark --capture-baseline -v -s
