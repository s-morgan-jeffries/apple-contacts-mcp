...
- `packaging/Info.plist` — bundle metadata scaffold with `NSContactsUsageDescription` (the macOS TCC consent-dialog copy) and standard keys (`CFBundleIdentifier`, `CFBundleExecutable` matching the `[project.scripts]` console name, `LSUIElement=true` for headless operation, `LSMinimumSystemVersion=11.0`, `CFBundleShortVersionString` / `CFBundleVersion`). Not consumed by the current unbundled `uv run` launch path — macOS reads the Python interpreter's plist there. Ships ahead of any future bundling tool (py2app / briefcase / PyInstaller — deferred) so the artifact contract is fixed. `scripts/check_version_sync.sh` now enforces both Info.plist version strings against `pyproject.toml` via `plutil -extract`. `tests/unit/test_packaging.py` (7 tests) locks the required keys, the reverse-DNS bundle ID shape, the `LSUIElement` background-process flag, and the version-string match. README's `## Usage` section gained a concrete `claude_desktop_config.json` example. Closes #34 and the gap-analysis §3 bundling note.
- `scripts/check_tools_md_parity.sh` — enforces parity between `@mcp.tool()` registrations in `server.py` and `### <tool_name>` headings in `docs/reference/TOOLS.md`. Reports drift in either direction (undocumented tool or orphaned doc entry) and exits 1; unlike the sibling `check_client_server_parity.sh` there's no "internal helper" carve-out for a user-facing reference. Wired into `make check-all`, `test.yml`, and `release-hygiene.yml` alongside the existing parity check (#91, closes #51).
